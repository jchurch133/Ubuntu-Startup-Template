...
# Function to restore Chrome Windows (focuses Chrome window then presses Enter)
restore_chrome() {
    chrome_window_id=$(xdotool search --onlyvisible --name "Google Chrome")
    xdotool windowactivate --sync "$chrome_window_id" key Return &
    sleep 5
}

# Function to open the sound volume control (using a manually created shortcut in Kubuntu Linux)
open_sound_volume_control() {
    xdotool keydown Super_L key v keyup Super_L
}
